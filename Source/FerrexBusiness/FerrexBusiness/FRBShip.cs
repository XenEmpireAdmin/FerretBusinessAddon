...
{
    public class FRBShip : PassingShip, ITrader, IThingHolder
    {
        ...
        public FRBShip()
        {}
            public FRBShip(TraderKindDef def)
            {
                this.def = def;
                this.things = new ThingOwner<Thing>(this);
                FRBShip.tmpExtantNames.Clear();
                List<Map> maps = Find.Maps;
                for (int i = 0; i < maps.Count; i++)
                {
                    FRBShip.tmpExtantNames.AddRange(from x in maps[i].passingShipManager.passingShips
                                                      select x.name);
                }
                this.name = NameGenerator.GenerateName(RulePackDefOf.NamerTraderGeneral, FRBShip.tmpExtantNames, false);
